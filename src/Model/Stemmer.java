...
class Stemmer {
   ...
   */
   ...
   /** step3() maps double suffices to single ones. so -ization ( = -ize plus
    ...
    private String myStem (String term){

        String termAfterStermming = "";
        for(int index=0; index<term.length();index++){
            add(term.charAt(index));
        }
        stem();
        for(int index=0; index<i_end ;index++){
            termAfterStermming = termAfterStermming + b[index];
        }
    ...
    public Hashtable<String, LinkedList<Pair<String, Integer>>> stemDictionary(Hashtable <String, LinkedList<Pair<String, Integer>>> dictionary){
        ...
        for(String key: dictionary.keySet()){
            String stemTerm = myStem(key);
            if(dictionaryAfterStemming.containsKey(stemTerm)){
                (dictionaryAfterStemming.get(stemTerm)).addAll(dictionary.get(key));
            }
            else{
                dictionaryAfterStemming.put(stemTerm, dictionary.get(key));
            }
